...
public class CustomerManager {
	...
	public void guest() {
		...
		customerOrderNo[customerNo] = maxOrderNo;	//손님 주문갯수 저장
		// 음식 주문
		// 손님 객체 존재시 (1,2,3번(4번) 자리 지정)
		// 히든손님 아닐시
	...
	public synchronized void hidden(int customerNo) {
		...
		cTimer[customerNo] = new CustomerTimer(this,(11-(0.3*stageLv))/2,customerNo,customerX[customerNo]); // 각 손님별 타이머 설정
		...
		for (int i = 0; i < menuNo; i++) { // 메뉴 개수에 따라 반복
			...
			if (notHidden == false) { //히든손님 지불 가격
				if (random == 0) {
					...
					orderDao.addOrder(new MenuOrder("음료수", 3000, orderNo));
				} else if (random == 2) {
				...
				if (random == 0) {
					...
					orderDao.addOrder(new MenuOrder("음료수", 1500, orderNo));
				} else if (random == 2) {
