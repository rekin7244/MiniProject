...
/*import com.kh.miniProject.view.StageTimer.TimerStart;*/
	...
	public void guest() {
		...
		if(stageLv>4) {	//5stage부터 히든 출현
			if (count == 5) {
				...
				gP.add(mt);
				Image icon = new ImageIcon("images/Inked히든손님2.png").getImage().getScaledInstance(150, 220, 0); // 손님 이미지
				...
				Image[] icon = {new ImageIcon("images/손놈2.png").getImage().getScaledInstance(190, 250, 0),
						...
						new ImageIcon("images/손놈3.png").getImage().getScaledInstance(190, 250, 0)
				}; // 손님 이미지
		...
		/*customer[customerNo].setBounds(0, 105, 120, 200); // 손님 위치 설정
*/		customer[customerNo].setSize(120,200);
	...
	public void addOrder(int menuNo, int x, boolean guest) {
		...
		for (int i = 0; i < menuNo; i++) { // 메뉴 개수에 따라 반복
			...
			Image food = null;
			if (random == 0) {
				food = new ImageIcon("images/tbk1.png").getImage().getScaledInstance(50, 80, 0);
			} else if (random == 1) {
				food = new ImageIcon("images/drink1.png").getImage().getScaledInstance(50, 50, 0);
			} else if (random == 2) {
				food = new ImageIcon("images/fried1.png").getImage().getScaledInstance(50, 80, 0);
			} else if (random == 3) {
				food = new ImageIcon("images/오뎅.jpg").getImage().getScaledInstance(50, 80, 0);
			} else if (random == 4) {
				food = new ImageIcon("images/ramen.png").getImage().getScaledInstance(50, 80, 0);
			}
			...
			orderLabel[orderNo].setBounds(x + 120, y + 120, 70, 70);
			y += 40;
